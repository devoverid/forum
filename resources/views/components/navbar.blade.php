    ...
    <div class="flex justify-between h-full items-center relative">
        ...
        <div class="navbar-links xl:flex-grow relative hidden md:flex md:justify-center text-xs lg:text-sm xl:w-1/2">
            ...
            @foreach ($links as $link)
                <a href="{{ $link['link'] }}" class="navbar-link text-gray-300 border-b-8 border-transparent hover:border-white m-3 md:px-1 xl:px-0 py-4 pt-6 block uppercase text-transparent-50 font-semibold hover:text-white {{ request()->is( str_replace(url('') . '/', '', $link['link'] ) . '*') ? 'active' : '' }}">
                    {{ $link['text'] }}
                </a>                
            @endforeach
        </div>
